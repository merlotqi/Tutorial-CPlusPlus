...
int main()
{
    ...
    auto renderer = Renderer(quit, [&] {
        return vbox({
                text("Select changed: " + std::to_string(selection_change_counter) + " times"),
                text("Currently selected: "),
                paragraph(selection_content) | vscroll_indicator | frame | border | size(HEIGHT, EQUAL, 10),
                ...
                window(text("Grid split with different style"),
                       vbox({
                               ...
                               hbox({
                                       ...
                                       separator(),
                                       LoremIpsum() |
                                               selectionStyle([](Pixel &pixel) { pixel.underlined_double = true; }),
                                       separator(),
                                       LoremIpsum(),
                               }),
                       })),
