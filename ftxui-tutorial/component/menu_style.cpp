...
Component VMenu3(std::vector<std::string> *entries, int *selected)
{
    auto option = MenuOption::Vertical();
    option.entries_option.transform = [](EntryState state) {
        Element e = state.active ? text("[" + state.label + "]") : text(" " + state.label + " ");
        if (state.focused)
        {
            e = e | bold;
        }
...
Component VMenu4(std::vector<std::string> *entries, int *selected)
{
    ...
    option.entries_option.transform = [](EntryState state) {
        ...
        {
            return text(state.label) | color(Color::Yellow) | bgcolor(Color::Black) | bold;
        }
        ...
        if (state.focused)
        {
            return text(state.label) | color(Color::Black) | bgcolor(Color::Yellow) | bold;
        }
...
{
    auto option = MenuOption::Vertical();
    option.entries_option.animated_colors.foreground.Set(Color::Red, Color::White, std::chrono::milliseconds(500));
    return Menu(entries, selected, option);
}
